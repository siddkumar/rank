        ...
        <div className="round-container">
            {
                props.matchupList.map((item,index) => { 
                    ...
                    return (
                        <div key={index + "." + x.name + "vs" + y.name + "." + props.roundNumber}className="matchup-container">
                            <div key={x.name + x.rank + "." + props.roundNumber} className="row card item-container">
                                <div>{getNiceString(x.name, x.rank)}</div>
                                <div className="controls">
                                    <div onClick={() => props.clickCallback(psuedoSeed, x, props.roundNumber - 1)}>
                                    <i className="fa-solid fa-square-caret-right"></i>
                                    </div>
                                </div>
                            </div>
                            <div key={y.name + y.rank + props.roundNumber} className="row card item-container">
                                <div>{getNiceString(y.name, y.rank)}</div>
                                <div className="controls">
                                    <div onClick={() => props.clickCallback(psuedoSeed, y, props.roundNumber - 1)}>
                                    <i className="fa-solid fa-square-caret-right"></i>
                                    </div>
                                </div>
                            </div>
