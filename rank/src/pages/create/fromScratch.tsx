          ...
          <div className="main-subtitle">Rankable Things:</div>
          {
            items.map(
              (item, i) => 
                ...
                <button key={i+"-button-key"} className="close" onClick={(e) => removeItem(i)}>remove </button>
                </div>
              )
          }
          <button onClick={addItem} className="button-styles">Add Item</button>
        <button onClick={submitTemplate} className="button-styles">i'm done</button>
      </div>
    );
  }
  ...
  function readyView() {
    return (
      <div className="create-page-layout">
        ...
        <a href={"/rank?templateId=" + templateId}><button className="button-styles">let's rank</button></a>
      </div>
    );
  }
